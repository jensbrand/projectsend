...
//				symlink($sendfile, $secretDir . utf8_decode($filename));
				symlink($sendfile, $secretDir . $filename);
				
				// create web path
				if (!empty(self::$_linkDirUri))
				{
	//				$weburi = self::$_linkDirUri . $secret . '/' . utf8_decode($filename);
	...
	**/
	public static function runGBC()
	{
		...
		{
			error_log("runGBC Processing directory " . $linkdir . " filemtime directory " . filemtime($linkdir) . "actual time  " . time());
			
			// check creation date
			if (filemtime($linkdir) > (time() - self::$_expireTime))
			{
				error_log("runGBC directory is to young " . (time() - filemtime($linkdir)));
				continue;
			}
			
			// delete all files in dir
			error_log("runGBC deleting files in " . $linkdir);
			$filesToDelete = glob($linkdir . '/*');
			error_log( print_r($filesToDelete, true ) );
			
			array_walk($filesToDelete, array(__CLASS__, 'unlink'));
			
			// and the dir itself
			error_log("runGBC deleting directory " . $linkdir);
			rmdir($linkdir);
			$result = true;
		}
	...
	{
		error_log("runGBC deleting file " . $file);
		return unlink($file);
	}
}
?>